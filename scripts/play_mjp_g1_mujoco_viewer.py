#!/usr/bin/env python3
"""Play a native MuJoCo Playground G1 policy in the MuJoCo viewer."""
...
DEFAULT_CHECKPOINT = (
    REPO_ROOT
    / "models/mujoco_playground/g1_flat_seed0/G1JoystickFlatTerrain/seed0-0503-053829/step48829"
)


def _actor_input_dim(checkpoint: Path) -> int:
    ckpt = torch.load(checkpoint / "actor.pt", map_location="cpu")
    state_dict = ckpt["network_state_dict"]
    for key, value in state_dict.items():
        if key.endswith("embedder.w.w.weight"):
            return int(value.shape[1])
    raise KeyError(f"Could not infer actor input dim from {checkpoint / 'actor.pt'}")


def _load_actor(checkpoint: Path, input_dim: int, action_dim: int, device: torch.device) -> FlashSACActor:
    actor = FlashSACActor(
        num_blocks=2,
        input_dim=input_dim,
        hidden_dim=128,
...
def main() -> None:
    ...
    if hasattr(cfg, "push_config"):
        cfg.push_config.enable = False
        cfg.push_config.magnitude_range = [0.0, 0.0]
    env = registry.load(args.env_name, config=cfg)

    input_dim = _actor_input_dim(checkpoint)
    action_dim = env.action_size
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    actor = _load_actor(checkpoint, input_dim=input_dim, action_dim=action_dim, device=device)
